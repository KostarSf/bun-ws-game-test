...
export class EventEmitter<TEventMap extends EventMap = EventMap> {
	...
	private _empty = true;
	private _listeners: Record<string, Handler<unknown>[]> = {};
	private _listenersOnce: Record<string, Handler<unknown>[]> = {};
	private _pipes: EventEmitter<EventMap>[] = [];
	...
	on<TEventName extends EventKey<TEventMap> | string>(
		eventName: TEventName,
		handler: Handler<TEventMap[TEventName]>,
	): Subscription {
		this._empty = false;
	...
	once<TEventName extends EventKey<TEventMap> | string>(
		eventName: TEventName,
		handler: Handler<TEventMap[TEventName]>,
	): Subscription {
		this._empty = false;
	...
	off<TEventName extends EventKey<TEventMap> | string>(
		eventName: TEventName,
		handler?: Handler<TEventMap[TEventName]>,
	): void {
		if (handler) {
	...
	emit<TEventName extends EventKey<TEventMap> | string>(
		eventName: TEventName,
	 ...
	 * @param emitter
	 */
	pipe(emitter: EventEmitter<EventMap>): Subscription {
		if (this === emitter) {
	 ...
	 * @param emitter
	 */
	unpipe(emitter: EventEmitter<EventMap>): void {
		const i = this._pipes.indexOf(emitter);
		if (i > -1) {
			this._pipes.splice(i, 1);
		}
